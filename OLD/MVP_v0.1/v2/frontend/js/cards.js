/* ══════════════════════════════════════════════
   ...
   Зависит от: state.js (App, UserTemplates, getUserTemplate, slotAspectCSS,
               slotsConfigFromCard, addUserTemplate, esc, getActiveProject, CAT_NAMES)
...
function cpRenderList() {
  ...
  for (var i = 0; i < proj.cards.length; i++) {
    ...
    var catShort = CAT_NAMES[c.category] || '';
    var fileCount = 0;
    if (c.slots) {
    ...
    html += 'Карточка ' + (i + 1);
    if (catShort) html += ' <span class="cat">' + esc(catShort) + '</span>';
...
function cpRenderCard() {
  ...
  /* Категория: тоггл-кнопки */
  var categories = cpGetCategories(proj);
  if (categories.length > 0) {
    html += '<div class="cp-categories">';
    for (var ci = 0; ci < categories.length; ci++) {
      var catKey = categories[ci].key;
      var catName = categories[ci].name;
      var catActive = card.category === catKey ? ' active' : '';
      html += '<button class="cat-btn' + catActive + '" onclick="cpToggleCategory(\'' + catKey + '\')">' + esc(catName) + '</button>';
    }
    html += '</div>';
  }
...
/**
 * Получить список категорий: merge дефолтных CAT_NAMES с proj.categories.
 * @param {Object} proj
 * @returns {Array<{key: string, name: string}>}
 */
function cpGetCategories(proj) {
  var cats = [];
  var keys = Object.keys(CAT_NAMES);
  for (var i = 0; i < keys.length; i++) {
    cats.push({ key: keys[i], name: CAT_NAMES[keys[i]] });
  }
  if (proj.categories && proj.categories.length) {
    for (var j = 0; j < proj.categories.length; j++) {
      var custom = proj.categories[j];
      var exists = false;
      for (var k = 0; k < cats.length; k++) {
        if (cats[k].key === custom || cats[k].name === custom) { exists = true; break; }
      }
      if (!exists) cats.push({ key: custom.toLowerCase(), name: custom });
    }
  }
  return cats;
}
...
/**
 * Переключить категорию карточки (тоггл).
 * @param {string} key
 */
function cpToggleCategory(key) {
  var card = getActiveProject().cards[App.currentCardIdx];
  card.category = (card.category === key) ? '' : key;
  cpRenderList();
}
...
function cpOpenTemplateEditor() {
  ...
  /* hasHero: из карточки или шаблона */
  ...
  slots.forEach(function(el) {
    ...
    el.addEventListener('drop', function(e) {
      ...
      if (cpDragSourceSlot !== null && cpDragSourceSlot !== targetIdx) {
        ...
        var tmpFile = tgt.file, tmpData = tgt.dataUrl, tmpPath = tgt.path;
        tgt.file = src.file; tgt.dataUrl = src.dataUrl; tgt.path = src.path;
        src.file = tmpFile; src.dataUrl = tmpData; src.path = tmpPath;
        cpSyncFiles(card);
        cpRenderCard();
        cpRenderList();
      }
...
function cpExportList() {
  ...
  for (var i = 0; i < proj.cards.length; i++) {
    var c = proj.cards[i];
    lines.push('--- Карточка ' + (i + 1) + ' (' + (CAT_NAMES[c.category] || c.category || '') + ') ---');
    if (c.slots) {
