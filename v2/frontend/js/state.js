...
/* hint — короткое пояснение к этапу: что делать и какие инструменты доступны.
   Показывается раскрывающимся блоком под активным шагом пайплайна (задача 1.14). */
var PIPELINE_STAGES = [
  { id: 'preselect',    name: 'Превью (преотбор)',      beta: true,
    hint: 'Загрузите превью из папки Capture One и отсейте технический брак. Рейтинг и ключевые слова проставляются в COS через Rate Setter — они приезжают в превью автоматически.' },
  { id: 'selection',    name: 'Отбор команды',          beta: true,
    hint: 'Соберите отбор команды: перетащите лучшие кадры в слоты карточек. Когда отбор готов — отправьте клиенту ссылку на преотбор кнопкой «Ссылка на преотбор».' },
  { id: 'client',       name: 'Отбор клиента',          beta: true,
    hint: 'Клиент открывает ссылку и отмечает финальные кадры. Дождитесь согласования; если клиент вернул отбор — доработайте и отправьте снова.' },
  { id: 'color',        name: 'Цветокоррекция',         beta: false,
    hint: 'Загрузите цветокоррекцию (ЦК-версию) кнопкой «Загрузить ЦК». Сравните с превью; при расхождениях верните на доработку.' },
  { id: 'retouch_task', name: 'Комментарии на ретушь',  beta: false,
    hint: 'Оставьте комментарии и аннотации на кадрах — что править ретушёру. При необходимости выгрузите ТЗ на ретушь. Комментирование идёт параллельно и не двигает этап.' },
  { id: 'retouch',      name: 'Ретушь',                 beta: false,
    hint: 'Загрузите отретушированные версии кнопкой «Загрузить ретушь» и проверьте, что правки соответствуют комментариям.' },
  { id: 'retouch_ok',   name: 'Согласование ретуши',    beta: false,
    hint: 'Согласуйте ретушь с клиентом. После подтверждения переходите к адаптации или сдавайте проект.' },
  { id: 'adaptation',   name: 'Адаптация к каналам',    beta: false,
    hint: 'Подготовьте кадры под площадки публикации (кропы и размеры каналов). Финальный шаг — сдайте проект клиенту кнопкой «Сдано клиенту».' },
];
