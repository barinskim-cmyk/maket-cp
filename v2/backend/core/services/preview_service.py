...
THUMB_MAX_SIZE = 300  # макс. сторона миниатюры (px) — для галереи
THUMB_QUALITY = 70  # JPEG качество для thumb
PREVIEW_MAX_SIZE = 1200  # макс. сторона для карточек (чёткое превью)
PREVIEW_QUALITY = 85  # JPEG качество для preview
...
class PreviewItem:
    ...
    thumb: str = ""  # base64 JPEG миниатюра 300px (data:image/jpeg;base64,...)
    preview: str = ""  # base64 JPEG превью 1200px для карточек
...
class PreviewService:
    ...
    def _make_preview_pil(self, filepath: Path) -> PreviewItem | None:
        """Генерация миниатюр через Pillow (оптимальный путь).

        Создаёт две версии:
          thumb   — 300px для галереи (быстрая прокрутка)
          preview — 1200px для карточек и фулскрина (чёткое изображение)
        """
        try:
            ...
            # 1. Thumb (300px) для галереи
            t_ratio = min(THUMB_MAX_SIZE / orig_w, THUMB_MAX_SIZE / orig_h, 1.0)
            if t_ratio < 1.0:
                t_img = img.resize(
                    (max(1, int(orig_w * t_ratio)), max(1, int(orig_h * t_ratio))),
                    Image.LANCZOS,
                )
            else:
                t_img = img
            t_buf = io.BytesIO()
            t_img.save(t_buf, format="JPEG", quality=THUMB_QUALITY, optimize=True)
            thumb = f"data:image/jpeg;base64,{base64.b64encode(t_buf.getvalue()).decode('ascii')}"

            # 2. Preview (1200px) для карточек
            p_ratio = min(PREVIEW_MAX_SIZE / orig_w, PREVIEW_MAX_SIZE / orig_h, 1.0)
            if p_ratio < 1.0:
                p_img = img.resize(
                    (max(1, int(orig_w * p_ratio)), max(1, int(orig_h * p_ratio))),
                    Image.LANCZOS,
                )
            else:
                p_img = img
            p_buf = io.BytesIO()
            p_img.save(p_buf, format="JPEG", quality=PREVIEW_QUALITY, optimize=True)
            preview = f"data:image/jpeg;base64,{base64.b64encode(p_buf.getvalue()).decode('ascii')}"
