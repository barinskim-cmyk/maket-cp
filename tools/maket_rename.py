#!/usr/bin/env python3
"""
Content Pulse — Скрипт переименования фотографий по CSV-списку.

Использование:
  1. Экспортируйте CSV-список из Content Pulse (вкладка Артикулы -> Список на переименование)
...
# Принудительный UTF-8 для вывода (Windows cp1252/cp1251, CI-среды)
try:
...
def main():
    print("=" * 50)
    print("  Content Pulse — Переименование фотографий")
    print("=" * 50)
    ...
    log_lines.append(f"Content Pulse Rename Log — {timestamp}")
    ...
    try:
        with open("rename_log.txt", "w", encoding="utf-8") as f:
            f.write("\n".join(log_lines))
        print(f"Лог записан: rename_log.txt")
    except Exception:
        pass
