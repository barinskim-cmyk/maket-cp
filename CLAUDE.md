...
## Структура файлов (ревизия 2026-07-02)
...
4. **Verification до DONE.** Не помечать задачу как готовую без одного из:
   - запуска теста (Playwright / unit / smoke), который доказывает что фикс работает; **или**
   - демо-прокрутки через Chrome MCP (открыть live URL, воспроизвести сценарий, скриншот результата); **или**
   - явной проверки данных в Supabase (SELECT перед/после, edge function call), если правка SQL/RLS/edge-function.
   "Скомпилировалось без ошибок" — НЕ верификация. Если ни один из трёх способов невозможен — указать это в коммите явно, чтобы Masha знала что проверить руками.
...
## Правила автономной работы (с 2026-07-02)

Claude работает над проектом и без прямого участия Маши (запланированные сессии пн/ср/пт 9:00, сводка пт 16:00, keep-alive Supabase ежедневно).

**Claude делает сам, без вопросов:**
- Код, тесты, багфиксы, рефакторинг в рамках задач из backlog.html
- Коммиты и push (Desktop Commander: `git push origin main`; gh авторизован)
- Миграции и данные в Supabase — база тестовая, активных клиентов нет
- Обновление backlog.html, docs/, памяти

**Claude НЕ делает без явного «да» Маши:**
- Необратимое удаление данных или файлов (OLD/ не трогать)
- Всё, что стоит денег (тарифы, API-ключи, сервисы)
- Публикация наружу: лендинг, письма, посты, что-либо видимое клиентам
- Продуктовые и бизнес-решения (цены, позиционирование, приоритеты стратегии) — такие вопросы копить в еженедельную сводку с рекомендацией
- Изменение задач с пометкой «нужна Маша» в бэклоге

**Очередь работы:** backlog.html — единственный источник задач. Брать сверху по приоритету (P0→P1→P2). Тест FAIL/? от Маши — чинить в первую очередь. Одна сессия = минимум одна завершённая задача с верификацией.
